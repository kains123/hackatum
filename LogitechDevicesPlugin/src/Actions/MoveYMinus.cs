namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class MoveYMinus : PluginDynamicCommand
    {
        private int _counter = 0;
        private static readonly HttpClient Http = new HttpClient();

        public MoveYMinus()
            : base(displayName: "Move Y-", description: "Counts button presses", groupName: "Commands")
        {
        }
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[MoveYMinus] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"MoveYMinus\",\"count\":{count}}}";
                ...
                {
                    Console.WriteLine($"[MoveYMinus] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MoveYMinus] Error sending count: {ex}");
            }
        }
    }
}
