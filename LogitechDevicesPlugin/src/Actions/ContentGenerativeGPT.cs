namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class ContentGenerativeGPT : PluginDynamicCommand
    {
        private int _counter = 0;
        private static readonly HttpClient Http = new HttpClient();
        
        // 문제의 URL을 TargetUrl로 사용합니다.
        private const string TargetUrl = "https://chatgpt.com/g/g-h8l4uLHFQ-video-ai-by-invideo"; 

        public ContentGenerativeGPT()
            : base(displayName: "Content Generative GPT", description: "Opens AI Tool URL", groupName: "Commands")
        {
        }
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[ContentGenerativeGPT] Button Pressed: {_counter}");
            ...
            try
            {
                ...
                Console.WriteLine($"[ContentGenerativeGPT] Attempted to open URL: {TargetUrl}");
            }
            catch (Exception ex)
            {
                // 실패 시 콘솔에 상세한 에러 로그를 남깁니다.
                Console.WriteLine($"[ContentGenerativeGPT] Error opening URL: {ex.Message}");
        ...
        protected override string GetCommandDisplayName(string actionParameter, PluginImageSize imageSize) =>
            $"Content \nGenerative \nGPT {Environment.NewLine}";
            
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"ContentGenerativeGPT\",\"count\":{count}}}";
