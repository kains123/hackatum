namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class MoveZPlus : PluginDynamicCommand
    {
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[MoveZPlus] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"MoveZPlus\",\"count\":{count}}}";
                ...
                {
                    Console.WriteLine($"[MoveZPlus] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MoveZPlus] Error sending count: {ex}");
            }
        }
    }
}
