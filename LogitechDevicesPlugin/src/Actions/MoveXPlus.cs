namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class MoveXPlus : PluginDynamicCommand
    {
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[MoveXPlus] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"MoveXPlus\",\"count\":{count}}}";
                ...
                {
                    Console.WriteLine($"[MoveXPlus] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MoveXPlus] Error sending count: {ex}");
            }
        }
    }
}
