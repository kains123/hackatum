namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class MoveYPlus : PluginDynamicCommand
    {
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[MoveYPlus] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"MoveYPlus\",\"count\":{count}}}";
                ...
                {
                    Console.WriteLine($"[MoveYPlus] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MoveYPlus] Error sending count: {ex}");
            }
        }
    }
}
