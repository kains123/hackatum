namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class MoveZMinus : PluginDynamicCommand
    {
        private int _counter = 0;
        private static readonly HttpClient Http = new HttpClient();

        public MoveZMinus()
            : base(displayName: "Move Z-", description: "Counts button presses", groupName: "Commands")
        {
        }
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[MoveZMinus] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[MoveZMinus] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MoveZMinus] Error sending count: {ex}");
            }
        }
    }
}
