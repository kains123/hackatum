namespace Loupedeck.LogitechDevicesPlugin
{
    ...
    public class Delete : PluginDynamicCommand
    {
        private int _counter = 0;
        private static readonly HttpClient Http = new HttpClient();

        public Delete()
            : base(displayName: "Delete", description: "Counts button presses", groupName: "Commands")
        {
        }
        ...
        protected override void RunCommand(string actionParameter)
        {
            ...
            Console.WriteLine($"[Delete] Counter value is {_counter}");
        ...
        private static async Task SendCountAsync(int count)
        {
            try
            {
                var json = $"{{\"type\":\"Delete\",\"count\":{count}}}";
                ...
                {
                    Console.WriteLine($"[Delete] HTTP error: {(int)response.StatusCode} {response.ReasonPhrase}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Delete] Error sending count: {ex}");
            }
        }
    }
}
